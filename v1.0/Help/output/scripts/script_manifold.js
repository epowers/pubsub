...
function HideSelect()
{
	var selectTags = document.body.all.tags("SELECT");
	var spanEles = document.body.all.tags("span");
	var i = 10;
	var m;
	...
	if (selectTags.length != null || selectTags.length >0)
	{
		...
		{			
			var lan = selectTags(n).id.substr("10");
			...
			switch (lan.toLowerCase())
			{
				case "visualbasic":
					...
					{					
						if (spanEles[m].codeLanguage == "VisualBasic" && spanEles[m].style.display != "none" && n <i)
							i = n;				
					}
					break;
				case "visualbasicdeclaration":
					for (m=0; m<spanEles.length; m++)
					{					
						if (spanEles[m].codeLanguage == "VisualBasicDeclaration" && spanEles[m].style.display != "none" && n < i)
							i = n;
					}
					break;
				case "visualbasicusage":
					...
					for (m=0; m<spanEles.length; m++)
					{					
						if (spanEles[m].codeLanguage == "VisualBasicUsage" && spanEles[m].style.display != "none" && n <i)
							i = n;				
					}
					break;
				case "csharp":
					for (m=0; m<spanEles.length; m++)
					{					
						if (spanEles[m].codeLanguage == "CSharp" && spanEles[m].style.display != "none" && n < i)
							i = n;
					}
					break;
				case "managedcplusplus":
					for (m=0; m<spanEles.length; m++)
					{					
						if (spanEles[m].codeLanguage == "ManagedCPlusPlus" && spanEles[m].style.display != "none" && n < i)
							i = n;
					}
					break;
				case "jsharp":
					for (m=0; m<spanEles.length; m++)
					{					
						if (spanEles[m].codeLanguage == "JSharp" && spanEles[m].style.display != "none" && n < i)
							i = n;
					}
					break;
				case "jscript":
					for (m=0; m<spanEles.length; m++)
					{					
						if (spanEles[m].codeLanguage == "JScript" && spanEles[m].style.display != "none" && n < i)
							i = n;
					}
					break;				
				case "xaml":
					...
					for (m=0; m<spanEles.length; m++)
					{					
						if (spanEles[m].codeLanguage == "XAML" && spanEles[m].style.display != "none" && n <i)
							i = n;				
					}
					break;
			}							
...
// AlanSm: start custom code
			if(spanElements[i].codeLanguage == "XAML")
			{
				if(xamlLang == "on")
					spanElements[i].style.display = "";
				else
...
{
	var iAllMembers = document.all.tags("tr");
	var i;
	
	for(i = 0; i < iAllMembers.length; ++i)
	{
		if (((iAllMembers[i].protected == "true") && (protectedMembers == "off")) ||
			((iAllMembers[i].name == "inheritedMember") && (inheritedMembers == "off")) ||
			((iAllMembers[i].notSupportedOn == "netcf") && (netcfMembersOnly == "on")))
...
function sendfeedback(subject, id,alias){
	...
	var title = document.all.tags("TITLE")[0].innerText.replace(rExp, "''");
