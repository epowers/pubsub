...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1">
	...
	<xsl:template match="/">
		<HelpTOC DTDVersion="1.0">
			<xsl:choose>
				<xsl:when test="count(/*/apis/api[apidata/@group='root']) &gt; 0">
					<xsl:apply-templates select="/*/apis/api[apidata/@group='root']" />
				</xsl:when>
				<xsl:when test="count(/*/apis/api[apidata/@group='namespace']) &gt; 0">
					<xsl:apply-templates select="/*/apis/api[apidata/@group='namespace']">
						<xsl:sort select="apidata/@name" />
					</xsl:apply-templates>
				</xsl:when>
				<xsl:otherwise>
					<xsl:apply-templates select="/*/apis/api[apidata/@group='type']">
						<xsl:sort select="apidata/@name" />
					</xsl:apply-templates>
				</xsl:otherwise>
			</xsl:choose>
